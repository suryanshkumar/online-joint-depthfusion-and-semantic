...
class Extractor(nn.Module):
    ...
    def forward(self, depth, extrinsics, intrinsics, tsdf_volume, weights_volume, origin, resolution):
        '''
        ...
        if torch.cuda.is_available() and self.config.SETTINGS.gpu:
    ...
    @staticmethod
    def extract_rays(coords, eye, origin, resolution):
...
def trilinear_interpolation(points, tsdf_volume, weights_volume):
    ...
    value_container = -0.1 * torch.ones_like(valid).float() 
